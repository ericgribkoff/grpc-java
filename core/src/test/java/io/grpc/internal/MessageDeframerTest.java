...
public class MessageDeframerTest {
  ...
  public void smallCombinedPayloads() {
    ...
    verify(listener, atLeastOnce()).bytesRead(anyInt());
    assertEquals(Bytes.asList(new byte[] {14, 15}), bytes(producerCaptor.getValue().next()));
    verify(listener, atLeastOnce()).bytesRead(anyInt());
    assertNull(producerCaptor.getValue().next());
    verifyNoMoreInteractions(listener);
    checkStats(2, 3, 3);
  ...
  public void endOfStreamShouldNotifyEndOfStream() {
    ...
    verify(listener, times(1)).messagesAvailable(producerCaptor.capture());
    assertNull(producerCaptor.getValue().next());
    verify(listener).endOfStream();
    verifyNoMoreInteractions(listener);
    checkStats(0, 0, 0);
  ...
  public void payloadSplitBetweenBuffers() {
    ...
    assertEquals(
        Bytes.asList(new byte[] {3, 14, 1, 5, 9, 2, 6}), bytes(producerCaptor.getValue().next()));
    verify(listener, atLeastOnce()).bytesRead(anyInt());
    assertNull(producerCaptor.getValue().next());
