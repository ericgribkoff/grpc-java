...
@ThreadSafe // Hrm :/
public class StatsTraceContextImpl implements StatsTraceContext {
  public static final StatsTraceContextImpl NOOP = new StatsTraceContextImpl(new StreamTracer[0]);

  public static class ForwardingStatsTraceContext implements StatsTraceContext {
    private volatile StatsTraceContext delegate;

    ForwardingStatsTraceContext(StatsTraceContext delegate) {
      this.delegate = delegate;
    }

    void setDelegate(StatsTraceContext delegate) {
      this.delegate = delegate;
    }
    ...
    public Context.CancellableContext setInterceptorStreamTracersAndFilterContext(
      ...
      return delegate.setInterceptorStreamTracersAndFilterContext(newTracers, context);
    }

    @Override
    public List<StreamTracer> getTracersForTest() {
      return delegate.getTracersForTest();
    }

    @Override
    public void clientOutboundHeaders() {
      delegate.clientOutboundHeaders();
    }

    @Override
    public void clientInboundHeaders() {
      delegate.clientInboundHeaders();
    }

    @Override
    public void clientInboundTrailers(Metadata trailers) {
      delegate.clientInboundTrailers(trailers);
    }

    @Override
    public <ReqT, RespT> Context serverFilterContext(Context context) {
      return delegate.serverFilterContext(context);
    }

    @Override
    public void serverCallStarted(ServerCallInfo<?, ?> callInfo) {
      delegate.serverCallStarted(callInfo);
    }
  ...
  public static StatsTraceContext newClientContext(
    ...
    return new StatsTraceContextImpl(tracers);
  }

  /** Factory method for the server-side. */
  // TODO: delayed
  public static StatsTraceContextImpl newServerContext(
  ...
  @VisibleForTesting
  StatsTraceContextImpl(StreamTracer[] tracers) {
    this(tracers, false);
  }

  @VisibleForTesting
  StatsTraceContextImpl(StreamTracer[] tracers, boolean isServer) {
